...
if BOT_TOKEN is None:
    logger.exception("No BOT_TOKEN found in environment variables.")
    raise ValueError("No BOT_TOKEN found in environment variables.")

bot = Bot(token=BOT_TOKEN)
redis_url = getenv('REDIS_URL')
if redis_url is None:
    logger.exception("No REDIS_URL found in environment variables.")
...
async def main():
    ...
    await site.start()
    logger.info("HTTP server is running on http://tg-bot:9090")
